/**
 * API: Resolve Fraud Signal
 * PATCH /api/admin/referrals/fraud/[id]
 *
...
import { resolveFraudSignal } from '../../../../../services/admin-referral.service';
...
export const PATCH: APIRoute = async ({ params, request, cookies, clientAddress }) => {
  // Rate limiting
  const rateLimit = checkRateLimit(clientAddress, 'admin-fraud-resolve', 20, 60000);
  ...
  try {
    const { id } = params;
    if (!id) {
      return new Response(JSON.stringify({ error: 'Fraud signal ID required' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      });
    }
    ...
    const result = await resolveFraudSignal(id, adminSession.data.user.id);

    if (!result.success) {
      return new Response(JSON.stringify({ error: result.error || 'Failed to resolve fraud signal' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      });
    }
    ...
    await logAdminAction({
      ...
      action: 'referral.fraud_resolve' as any,
    ...
    console.error('[ADMIN API] Error resolving fraud signal:', error);
