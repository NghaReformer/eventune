/**
 * API: Update Sample
 * POST /api/admin/content/samples/update
 *
...
import { updateSample } from '../../../../../services/admin.service';
...
export const POST: APIRoute = async ({ request, cookies, clientAddress }) => {
  ...
  try {
    ...
    const { id, csrf_token, ...sampleData } = body;
    ...
    // Update sample
    const result = await updateSample(id, sampleData);

    if (!result.success) {
      return new Response(
        JSON.stringify({ error: result.error || 'Failed to update sample' }),
        { status: 400, headers: { 'Content-Type': 'application/json' } }
      );
    }
    ...
    console.error('[ADMIN] Sample update error:', error);
    return new Response(
