/**
 * API: Delete Sample
 * POST /api/admin/content/samples/delete
 *
...
import { deleteSample, getSampleById } from '../../../../../services/admin.service';
...
export const POST: APIRoute = async ({ request, cookies, clientAddress }) => {
  ...
  try {
    ...
    if (!id || typeof id !== 'string') {
      return new Response(
        JSON.stringify({ error: 'Invalid sample ID' }),
        { status: 400, headers: { 'Content-Type': 'application/json' } }
      );
    }

    // Get sample details for audit log
    ...
    if (!result.success) {
      return new Response(
        JSON.stringify({ error: result.error || 'Failed to delete sample' }),
        { status: 400, headers: { 'Content-Type': 'application/json' } }
      );
    }
    ...
    await logAdminAction({
      ...
      resource_id: id,
      metadata: { title: sample?.title, occasion_slug: sample?.occasion_slug },
    ...
    console.error('[ADMIN] Sample delete error:', error);
    return new Response(
