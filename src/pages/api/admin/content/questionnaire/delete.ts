/**
 * API: Delete Questionnaire Field
 * POST /api/admin/content/questionnaire/delete
 *
 * Security: Admin auth, content:delete permission, CSRF protection
 */

import type { APIRoute } from 'astro';
import { getAdminSession, hasPermission } from '../../../../../lib/auth/admin-session';
import { validateCSRFToken } from '../../../../../lib/auth/session';
import { deleteQuestionnaireField } from '../../../../../services/admin.service';
...
export const POST: APIRoute = async ({ request, cookies, clientAddress }) => {
  ...
  try {
    ...
    // Delete field
    const result = await deleteQuestionnaireField(id);

    if (!result.success) {
      return new Response(
        JSON.stringify({ error: result.error || 'Failed to delete field' }),
        { status: 400, headers: { 'Content-Type': 'application/json' } }
      );
    }
    ...
    await logAdminAction({
      ...
      action: 'questionnaire_field.delete',
    ...
    console.error('[ADMIN] Questionnaire field delete error:', error);
    return new Response(
