/**
 * API: Delete Occasion
 * POST /api/admin/content/occasions/delete
 *
...
import { deleteOccasion, getOccasionById } from '../../../../../services/admin.service';
...
export const POST: APIRoute = async ({ request, cookies, clientAddress }) => {
  ...
  try {
    ...
    // Get occasion details for audit log
    ...
    if (!result.success) {
      return new Response(
        JSON.stringify({ error: result.error || 'Failed to delete occasion' }),
        { status: 400, headers: { 'Content-Type': 'application/json' } }
      );
    }
    ...
    await logAdminAction({
      ...
      resource_id: id,
      metadata: { slug: occasion?.slug, name: occasion?.name },
