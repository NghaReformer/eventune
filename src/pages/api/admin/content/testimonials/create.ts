...
import { createTestimonial } from '../../../../../services/admin.service';
...
export const POST: APIRoute = async ({ request, cookies, clientAddress }) => {
  ...
  try {
    ...
    const { csrf_token, ...testimonialData } = body;
    ...
    if (!testimonialData.customer_name || !testimonialData.quote || !testimonialData.occasion) {
      return new Response(
        JSON.stringify({ error: 'Customer name, quote, and occasion are required' }),
    ...
    const result = await createTestimonial({
      ...
      customer_location: testimonialData.customer_location || null,
      quote: testimonialData.quote,
      occasion: testimonialData.occasion,
      rating: parseInt(testimonialData.rating) || 5,
      image_url: testimonialData.image_url || null,
      video_url: testimonialData.video_url || null,
      video_source_type: testimonialData.video_source_type || 'youtube',
      is_featured: testimonialData.is_featured === true,
      is_active: testimonialData.is_active !== false,
      display_order: parseInt(testimonialData.display_order) || 10,
    });

    if (!result.success) {
      return new Response(
        JSON.stringify({ error: result.error || 'Failed to create testimonial' }),
        { status: 400, headers: { 'Content-Type': 'application/json' } }
      );
    }
    ...
    await logAdminAction({
      ...
      metadata: { customer_name: testimonialData.customer_name, occasion: testimonialData.occasion },
